.onLoad <- function(libname="crushingr", pkgname="crushingr"){
  ...
  theme_clark <- function(base_size = 12, base_family = "") {
    ...
    theme(
      ...
      axis.title=          element_text(size = base_size * 1.5, vjust=0.35),
      strip.text =         element_text(size = rel(0.8)),

      axis.line =          element_line(size = 0.8),
      axis.line.x =        NULL,
      axis.line.y =        NULL,
      axis.text.x =        element_text(size = base_size*1.2, lineheight = 0.9,
                                        margin = margin(t = 0.8 * half_line/2), vjust = 1),
      axis.text.x.top =    element_text(margin = margin(b = 0.8 * half_line/2), vjust = 0),
      axis.text.y =        element_text(size = base_size*1.2, lineheight = 0.9,
      ...
      strip.text.x =       element_text(size = base_size * 1.6, margin = margin(t = half_line, b = half_line), face = "bold"),
      strip.text.y =       element_text(size = base_size * 1.6, angle = -90, margin = margin(l = half_line, r = half_line), face = "bold"),
